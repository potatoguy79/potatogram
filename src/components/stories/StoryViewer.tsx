...
import { X, ChevronLeft, ChevronRight, User, Heart, Send } from 'lucide-react';
      ...
      <div className="relative w-full max-w-md h-full max-h-[90vh] bg-black">
        ...
        <div className="absolute top-10 left-4 right-4 z-40 flex items-center gap-3">
          ...
          {currentStory.is_close_friends_only && (
            <span className="text-xs bg-close-friends text-white px-2 py-0.5 rounded-full">
              Close Friends
            </span>
          )}
        </div>
        ...
        <button
          onClick={goPrev}
          className="absolute left-0 top-1/2 -translate-y-1/2 w-1/3 h-2/3 z-30"
        />
        <button
          onClick={goNext}
          className="absolute right-0 top-1/2 -translate-y-1/2 w-1/3 h-2/3 z-30"
        />
        ...
        <div className="absolute bottom-4 left-4 right-4 z-40 flex items-center gap-3">
          <input
            ...
            className="flex-1 bg-transparent border border-white/30 rounded-full px-4 py-2 text-white text-sm placeholder:text-white/50 focus:outline-none focus:border-white/50"
          />
        ...
        {currentIndex > 0 && (
          <button
            onClick={goPrev}
            className="absolute left-2 top-1/2 -translate-y-1/2 p-1 bg-white/10 rounded-full hover:bg-white/20 transition-colors z-40"
          >
        ...
        {currentIndex < stories.length - 1 && (
          <button
            onClick={goNext}
            className="absolute right-2 top-1/2 -translate-y-1/2 p-1 bg-white/10 rounded-full hover:bg-white/20 transition-colors z-40"
          >
